...
/**
 * Generates a compact text representation of the board for side-by-side layout.
 * Optimized for the new DoubleTextWall layout that displays content side by side.
 * @param state - The current SessionState.
 * @param options - Rendering options
 * @returns The compact board as a string.
 */
export function renderCompactBoardString(
    state: SessionState,
    options?: {
        flipBoard?: boolean,
        showCoordinates?: boolean,
    ...
    const { board, userColor, capturedByWhite, capturedByBlack, lastMove } = state;
    // Defaults optimized for side-by-side layout
    const flipBoard = options?.flipBoard ?? (userColor === PlayerColor.BLACK);
    const showCoordinates = options?.showCoordinates ?? false; // Disable coordinates to save space
    const highlightLastMove = options?.highlightLastMove ?? false; // Disable highlighting to save space
    const showCapturedPieces = options?.showCapturedPieces ?? false; // Disable captured pieces to save space
    const useUnicode = options?.useUnicode ?? true;
    ...
    // Use minimal spacing for compact layout
    const pieceSpacing = ' ';
    const squareSpacing = ' ';

    let boardStr = "";

    // Board ranks and pieces (compact rendering)
    for (let r = 0; r < 8; r++) {
        const displayRow = flipBoard ? (7 - r) : r;
        let rowStr = showCoordinates ? `${8 - displayRow} ` : '';
        for (let c = 0; c < 8; c++) {
    ...
    // File labels (only if coordinates are enabled)
    if (showCoordinates) {
        boardStr += '  ';
        for (let c = 0; c < 8; c++) {
            boardStr += `${FILES[c]} `;
...
/**
 * Generates an ultra-compact text representation of the board for very constrained displays.
 * Uses minimal spacing and no coordinates for maximum space efficiency.
 * @param state - The current SessionState.
 * @param options - Rendering options
 * @returns The ultra-compact board as a string.
 */
export function renderUltraCompactBoardString(
    state: SessionState,
    options?: {
        flipBoard?: boolean,
        useUnicode?: boolean
    ...
    // Board ranks and pieces (ultra-compact rendering)
    for (let r = 0; r < 8; r++) {
        const displayRow = flipBoard ? (7 - r) : r;
        let rowStr = '';
        for (let c = 0; c < 8; c++) {
